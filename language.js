...
function getLangueage(event) {
  ...
  location.href = window.location.href =
    "user_account.html?id=" + userId + "#" + languageValue;
}
...
function changeLanguage() {
  ...
  if (!arrLanguage.includes(getHash)) {
    ...
    const userId = urlParams.get("id");
    location.href = window.location.href =
      "user_account.html?id=" + userId + "#brt";
  }
  const pageTitle = (document.querySelector("title").innerHTML =
    langArr["title"][getHash]);
  const date = (document.querySelector(".lng_date").textContent =
    langArr["date"][getHash]);
  const numid = (document.querySelector(".lng_numid").textContent =
    langArr["numid"][getHash]);
  const hour = (document.querySelector(".lng_hour").textContent =
    langArr["hour"][getHash]);
  const sum = (document.querySelector(".lng_sum").textContent =
    langArr["sum"][getHash]);
  const name = (document.querySelector(".lng_name").textContent =
    langArr["name"][getHash]);
  const login = (document.querySelector(".lng_login").textContent =
    langArr["login"][getHash]);
}
